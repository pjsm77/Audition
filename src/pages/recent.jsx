...
export default function Recent() {
  ...
  // --- 1. CARGA INICIAL DE DADOS (PAGINADA PARA VELOCIDADE MÁXIMA) ---
  useEffect(() => {
    async function initialLoad() {
      try {
        ...
        // Loop idêntico ao seu HTML para buscar blocos até garantir ao menos 10 dias de dados
        while (daysFound < 10 && offset < 2000) {
          const { data, error } = await supabase
            .from('recent_scrobbles_view') // Nome correto da view mapeado do seu HTML
            .select('*')
        ...
        // Gatilho em segundo plano para carregar todo o restante da base de dados silenciosamente
        loadFullLibrary();
      } catch (err) {
  ...
  // --- 2. AUXILIAR DE AGRUPAMENTO (ÚNICA FONTE DA VERDADE) ---
  const groupData = (dataSet) => {
    ...
    return Object.values(groups); // Retorna array de arrays de scrobbles
  };

  // --- 3. MONITORE O INPUT DE BUSCA (APLICA FILTRO DE ARTISTA) ---
  useEffect(() => {
    ...
    setCurrentPage(0); // Reseta para o primeiro dia após filtrar
  }, [searchTerm, allData]);

  // Grupos calculados em tempo de execução
  ...
  // --- 4. CONFIGURAÇÃO DO GRÁFICO DE BARRAS CUSTOMIZADO ---
  const chartData = {
  ...
  // Plugin idêntico ao Chart.js antigo para desenhar a quantidade acima das barras
  ...
  // --- 5. REDIRECIONAMENTO DE ÁLBUM NO DEEZER ---
  const openDeezer = (artist, album) => {
    ...
    window[cb] = (d) => {
      if (d.data && d.data[0]) {
        ...
        setTimeout(() => {
          if (document.hasFocus()) window.open(d.data[0].link, '_blank');
        }, 500);
      } else {
  ...
  if (loading) {
    return <div style={{ padding: '20px', color: '#888', backgroundColor: '#121212', height: '100vh' }}>Carregando scrobbles recentes...</div>;
  }
  ...
  return (
    <div style={{ backgroundColor: '#121212', color: '#e0e0e0', fontFamily: 'Segoe UI, Roboto, sans-serif', minHeight: '100vh', padding: '10px', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: '1000px', margin: 'auto' }}>
        ...
        {/* GRÁFICO DE EXECUÇÃO */}
        <div style={{ width: '100%', height: '140px', backgroundColor: '#1e1e1e', borderRadius: '8px', padding: '8px 5px 5px 5px', boxSizing: 'border-box', marginBottom: '10px' }}>
          {allData.length > 0 && (
            <Bar data={chartData} options={chartOptions} plugins={[customLabelsPlugin]} ref={chartRef} />
          )}
        </div>

        {/* BARRA DE FILTRO & DIAS */}
        <div style={{ backgroundColor: '#1e1e1e', padding: '10px', borderRadius: '8px', marginBottom: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            />
            {searchTerm && (
              <span onClick={() => setSearchTerm('')} style={{ position: 'absolute', right: '12px', cursor: 'pointer', color: '#888', fontWeight: 'bold', fontSize: '16px', userSelect: 'none' }}>×</span>
            )}
          </div>

          {/* ATALHOS RÁPIDOS DOS DIAS */}
          <div style={{ display: 'flex', gap: '4px', justifyContent: 'space-between' }}>
            {currentGroups.slice(0, 10).map((g, i) => {
              const dayButtonDate = new Date(g[0].date);
              return (
                <div
                  key={i}
        ...
        {/* LISTAGEM DOS SCROBBLES */}
        <div style={{ background: '#1e1e1e', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.5)' }}>
          {currentDayData ? (
            <>
              {/* HEADER DO DIA */}
              <div style={{ background: '#252525', padding: '8px 15px', fontWeight: 'bold', color: '#ba0000', textTransform: 'capitalize', borderBottom: '2px solid #ba0000', fontSize: '13px' }}>
              ...
              {currentDayData.map((item, idx) => (
                <div 
                  ...
                  style={{ display: 'flex', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid #2a2a2a', gap: '12px', cursor: 'pointer' }}
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    <div style={{ fontWeight: 'bold', color: '#fff', fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {item.song}
                    </div>
                    <div style={{ color: '#e0e0e0', fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {item.artist}
                    </div>
            ...
            <div style={{ padding: '20px', textAlign: 'center', color: '#888' }}>Nenhum dado encontrado para os filtros atuais.</div>
          )}
        </div>
