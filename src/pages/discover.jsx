...
// Inicialize o cliente do Supabase (substitua pelas suas variáveis de ambiente se necessário)
...
export default function Discover() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setError(null);
      try {
        // 1. Buscar dados da playlist do Deezer lidando com a paginação
        let tracks = [];
        let nextUrl = `https://api.deezer.com/playlist/${PLAYLIST_ID}/tracks`;
        
        // Contenção para o modo de desenvolvimento não entrar em loop infinito
        let pagesFetched = 0;
        const maxPages = 20; 

        while (nextUrl && pagesFetched < maxPages) {
          // Usando um proxy CORS público caso dê erro de CORS direto no navegador
          // Se sua aplicação já resolve CORS, pode remover o prefixo do proxy
          const response = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(nextUrl)}`);
          if (!response.ok) throw new Error('Falha ao buscar dados do Deezer.');
        ...
        // 2. Buscar IDs dos artistas na tbl_artists do Supabase
        const { data: supabaseArtists, error: sbError } = await supabase
          .from('tbl_artists')
          .select('deezer_id'); // Ajuste o nome da coluna se for diferente (ex: id_deezer)

        if (sbError) throw sbError;

        // Criar um Set com os IDs para busca rápida O(1)
  ...
  // 3. Cruzamento e Filtros de dados (otimizado com useMemo)
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500 text-red-500 p-4 rounded-lg my-4">
        Erros: {error}
      </div>
    );
  }
